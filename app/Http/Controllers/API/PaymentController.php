...
class PaymentController extends Controller
{
    ...
    {
        $query = Payment::where('member_id', '!=', NULL)->latest();
    ...
    //                 if ($product->wallet==0) {
    //                     $wallet = $the_member->bar_wallet;
    //                     $the_member->bar_wallet = $wallet - $request->amount;
    //                 }
    //                 elseif ($product->wallet==1) {
    //                     $wallet = $the_member->wallet_balance;
    //                     $the_member->wallet_balance = $wallete - $request->amount;
    //                 }
    //                 else {
    //                     $wallet = $the_member->credit_unit;
    //                     $the_member->credit_unit = $wallet - $request->amount;
    //                 }
    ...
    public function debitmembers(Request $request)
    {
        ...
        foreach ($request->people_id as $id){
            ...
            foreach ($members as $member) {
                ...
                $the_member = User::where('deleted_at', NULL)->where('unique_id', $member->membership_id)->first();
                ...
                if($the_member){
                    ...
                    if ($wallet >= $request->amount) {
                        $this->makeCredit($member->id, $payment_debit->id, $request->amount);
                        $payment_debit->status = 1;
                        $payment_debit->update();
                        $the_member->update();
                    }
